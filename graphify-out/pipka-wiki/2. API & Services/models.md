...
### Exclusions & Negative Keywords (Нововведение)
В модели `UserProfile` появилось поле `excluded_keywords` формата JSON/Array. 
Эта переменная хранит массив строк, которые пользователь ввел через дашборд. 
Она не мигрируется классическим `alembic` — для обеспечения бесперебойного продакшена добавлена логика `ALTER TABLE IF NOT EXISTS` в сервисе `database.py`. Это поле напрямую маршрутизируется в `rules.py` и `matcher.py` для отсечения работы.
