...
Без параметра `status` (вкладка «Все вакансии») **отклонённые скрываются**. Action переносится и на новую строку того же provider listing, если совпали `source + external_id` или точный URL. Отклонённые доступны на вкладке Rejected (`status=rejected`).
