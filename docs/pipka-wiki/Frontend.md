...
## Скрытые страны в основной ленте

Settings содержит вторую сетку стран — `#s-hidden-countries-grid`. Выбранные красные pills сохраняются в `UserProfile.hidden_countries`.

- Основная лента и Inbox по умолчанию не показывают выбранные страны.
- Сбор, дедупликация и AI-скоринг продолжаются.
- Явный выбор страны в верхнем multi-select временно переопределяет скрытие.
- Applied/Rejected остаются видимыми, чтобы не терять историю пользователя.

Переключение `hidden_countries` не меняет `profile_hash` и не запускает лишнюю перестройку embedding.
...
- **Не валидирует profile-формы перед отправкой.** Сервер ([[API]]) — единственный источник правды.
