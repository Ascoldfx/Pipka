...
- **TypeHandler group -1:** `_access_guard` до любого другого handler проверяет allowlist/active status пользователя.
...
→ [[Сервисы]] → [[Трекер]] → [[Скоринг]] → [[Watchlist]] → [[Frontend]] → [[База данных]]
